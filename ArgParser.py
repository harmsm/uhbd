...
class modeClass:
    ...
    def checkRequiredArgs(self):
        ...
        for index, arg in enumerate(self.required_arg_name):
            ...
            # Make sure specified output directory is a directory with pdb files
            if self.required_arg_type[index] == 'inpdir':
                if not os.path.isdir(self.required[arg]):
                    print "%s is not a valid directory!" % \
                        self.required[arg]
                    sys.exit()
                file_list = os.listdir(self.required[arg])
                file_list = [f for f in file_list if f[-4:] == ".pdb"]
                if len(file_list) == 0:
                    print "%s does not contain any pdb files!" % \
                        self.required[arg]
                    sys.exit()
