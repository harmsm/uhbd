...
# Constants
BIN_PATH = '/home/mharms1/uhbd-6.x.work/bin.gnu/'
...
def createTemporaryDirectory():
    ...
    global tmp_dir
    tmp_dir = "%stmp%i%s" % (script_path + os.sep,time.time(),os.sep)
    os.mkdir(tmp_dir)
    os.chdir(tmp_dir)
...
def runPrepares():
    ...
    # FIX SITESINPR (remove second line)
    ...
    # FIX TITRAA (can't remember what exactly is wrong with output... 1/12/06)
    f = open('titraa.pdb','r')
    titraa = f.readlines()
    f.close()
...
def fullRun(filename,pH_start,pH_stop,pH_interval,ionic_strength,dielectric):
